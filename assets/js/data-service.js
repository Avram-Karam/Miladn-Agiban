...
class DataService {
    ...
    static async _loadFromGAS() {
        ...
        // مهم: بيانات المشاركين الحالية للموقع تُؤخذ من ملفات المؤتمر المحلية
        // (المجموعات + التسكين) لمنع ظهور أسماء قديمة من Google Sheets.
        // Google Sheets يظل متاحاً للعمليات الإدارية/النقاط، لكنه ليس مصدراً
        // لأسماء المشاركين في واجهات الموقع العامة.
        structData.participants = this._buildLocalParticipants(structData);

        // لا نجلب قائمة Attendees القديمة من Google Sheets هنا.
        // هذا يمنع أي أسماء قديمة من الظهور في المجموعات/البحث/التسكين/اللوحات.
        ...
        /* الكود القديم لجلب Attendees من GAS متروك أدناه للتوافق مع وظائف الإدارة،
           لكنه لن يُنفذ في مسار تحميل بيانات الموقع العامة. */
        const directUrl = (window.YC_CONFIG && window.YC_CONFIG.DIRECT_GAS_URL) || '';
        let gasLoaded   = false;
        let gasError    = null;

        // 1. محاولة الـ Proxy. المشروع يدعم Netlify (/ _api/gas) وVercel (/api/gas)
        // لذلك نجرب المسارين تلقائياً بدلاً من افتراض منصة واحدة.
        if (!this._isGitHubPages()) {
            const proxyCandidates = [this.PROXY_URL, this.ALT_PROXY_URL].filter(Boolean)
                .filter((v, i, arr) => arr.indexOf(v) === i);
            for (const proxyUrl of proxyCandidates) {
                try {
                    const liveData = await this._fetchGASParticipants(proxyUrl);
                    if (liveData && Array.isArray(liveData)) {
                        console.log(`✅ جلب ${liveData.length} مشترك من Google Sheets عبر ${proxyUrl}`);
                    ...
                    gasError = err;
                    console.warn(`DataService: فشل Proxy ${proxyUrl} →`, err.message);
                }
            }
        }
