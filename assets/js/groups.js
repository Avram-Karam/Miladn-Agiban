...
(function () {
    ...
    async function init() {
        ...
        // حاول جلب النقاط فقط، من غير ما يسمح فشلها بإخفاء المجموعات.
        try {
            if (window.DataService && typeof DataService.fetchGroupScores === 'function') {
                const scoresData = await DataService.fetchGroupScores();
                if (Array.isArray(scoresData)) {
                    const map = new Map(scoresData.filter(x => x && x.group).map(x => [normalizeName(x.group), Number(x.grandTotal || 0)]));
                    groups.forEach(g => {
                        const key = normalizeName(g.name);
                        if (map.has(key)) g.overallPoints = map.get(key);
                    });
                }
            }
        } catch (e) {
            console.warn('groups.js: scores unavailable; showing groups without live scores:', e);
        }

        const TOTAL_MAX_CONFERENCE_POINTS = 365;
